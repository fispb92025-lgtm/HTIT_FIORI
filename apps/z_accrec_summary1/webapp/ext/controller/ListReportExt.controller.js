sap.ui.define([
    ...
    return {
        exportExcel: async function (oEvent) {
            const oSmartTable = this.getView().byId("zaccrecsummary::sap.suite.ui.generic.template.ListReport.view.ListReport::ZC_ACCREC_SUMMARY--listReport");
            if (!oSmartTable) {
            ...
            const companyCode = filteredData[0]?.RBUKRS || '6710'; // Use first row's company code or default to '6710'
            ...
            worksheet.getCell('A7').value = 'BÁO CÁO TỔNG HỢP CÔNG NỢ PHẢI THU';
            ...
            worksheet.getCell('A8').value = 'ACCOUNTS RECEIVABLE SUMMARY REPORT';
            ...
            // Xử lý màu riêng cho row 15 (VN đen, EN xanh)
            ...
            // ===== ÁP DỤNG ĐỒNG NHẤT VỚI BÁO CÁO PHẢI TRẢ =====
            // Các yêu cầu:
            // - Row 14: gạch dưới E14–P14
            // - Row 15: gạch dưới E15–P15
            // - Row 16: gạch dưới E16–P16
            // - Row 17: gạch dưới toàn bộ A17–Q17
            // - Row 17: gạch trên A17–D17 và Q17
            ...
            // Row 14 (tiếng Anh) - gạch dưới chỉ E14–P14
            worksheet.getRow(14).eachCell((cell, colNumber) => {
                if (colNumber >= 5 && colNumber <= 16) {  // E đến P
                    cell.border = {
                        top: { style: 'none' },
                        bottom: { style: 'medium' },
                        left: { style: 'medium' },
            ...
            // Row 15 (Nợ / Debit, Có / Credit) - gạch dưới chỉ E15–P15
            ...
            // Row 16 (Ngoại tệ, VND) - gạch dưới chỉ E16–P16
            ...
            // Row 17 (A B C D 1 2 3 ... 12 E) - gạch dưới toàn bộ
            // + gạch trên ở A17–D17 và Q17
            worksheet.getRow(17).eachCell((cell, colNumber) => {
                const isLeftPart = colNumber <= 4;   // A đến D
                const isRightPart = colNumber === 17; // Q
            ...
            // Đảm bảo viền trái/phải đầy đủ ở cột A-D và Q cho tất cả các row header
            ['A13', 'B13', 'C13', 'D13', 'Q13',
                'A14', 'B14', 'C14', 'D14', 'Q14',
                'A15', 'B15', 'C15', 'D15', 'Q15',
                'A16', 'B16', 'C16', 'D16', 'Q16',
            ...
            filteredData.forEach((item, idx) => {
                ...
                worksheet.getRow(rowIndex).eachCell((cell, colIndex) => {
                    ...
                    // Chỉ áp dụng định dạng số cho các cột tiền (5 đến 16)
                    if (colIndex >= 5 && colIndex <= 16) {
                        // Cột lẻ (5,7,9,11,13,15) = Ngoại tệ → có 2 thập phân
                        // Cột chẵn (6,8,10,12,14,16) = VND → không thập phân
                        cell.numFmt = (colIndex % 2 === 1) ? '#,##0.00' : '#,##0';
                    }
                });
            });
            ...
            for (let col = 5; col <= 16; col++) {
                ...
                worksheet.getCell(totalRowIndex, col).style = {
                    ...
                    border: { top: { style: 'medium' }, bottom: { style: 'medium' }, left: { style: 'medium' }, right: { style: 'medium' } },
                    numFmt: (col % 2 === 1) ? '#,##0.00' : '#,##0'  // Ngoại tệ có .00, VND không
                };
            }
            ...
            try {
                ...
                link.href = url;
                link.download = 'Export_VND_So_Tong_Hop_Cong_No_Phai_Thu.xlsx';
                link.click();
                // Clean up
        ...
        // };
        // // Default company information
        // let companyName = 'DEMO CÔNG TY TNHH CẢNG QUỐC TẾ TIL CẢNG HẢI PHÒNG';
        // let companyAddress = 'DEMO Khu CN Châu Sơn, Phường Châu Sơn, Tỉnh Ninh Bình, Việt Nam';
        // // Fetch company information from API
        // try {
        ...
        // worksheet.getCell('A4').value = 'SỔ TỔNG HỢP CÔNG NỢ PHẢI THU';
        // worksheet.getCell('A4').style = {
        ...
        // const link = document.createElement('a');
        // link.href = url;
        // link.download = 'Export_So_Tong_Hop_Cong_No_Phai_Thu.xlsx';
        // link.click();
        // // Clean up
        // window.URL.revokeObjectURL(url);
        // MessageToast.show("Export Successful!");
